        ...
        if (message.author.id !== message.guild.ownerId) {
            ...
            message.guild.channels.fetch().then(channels => {
                message.channel.send({ 
                    content:  `${message.author.toString()} select the channel you would like to set as default for patch notes.\nI can not see channels marked with ❌`,
                    components: [
                        {
                            'type': 1,
                            'components': [
                ...
                .then(select_msg => {
                    ...
                    const channel_filter = (click) => click.user.id === message.author.id && click.user.id === message.guild.ownerId && click.message.id === select_msg.id
                    ...
                    channel_collector.on("collect", async interaction => {
                        ...
                        const q = `REPLACE INTO Guilds (guildID, guildOwnerID, defaultChannelID) VALUES(${message.guildId}, ${message.guild.ownerId}, ${channel.id})`
